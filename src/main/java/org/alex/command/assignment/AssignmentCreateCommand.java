...
public class AssignmentCreateCommand extends AbstractCommand {
    ...
    public void execute(final Bootstrap bootstrap) throws Exception {
        ...
        if (choice == 1) {
            ...
            select = bootstrap.getNextInt();
            bootstrap.getAssignmentService().create(new Assignment(projects.get(select - 1).getUid(), idAssigner));
        } else {
            final List<Task> tasks = bootstrap.getTaskService().getAllTasks();
            i = 0;
            for (Task t : tasks) {
                i++;
                System.out.println(i + ". " + t.getName() + " : " + bootstrap.getAssigneeService().getAssigneeByUid(t.getOwnerId()));
            }
            System.out.print("Введите номер позиции: ");
            select = bootstrap.getNextInt();
            bootstrap.getAssignmentService().create(new Assignment(tasks.get(select - 1).getUid(), idAssigner));
        }
    }
